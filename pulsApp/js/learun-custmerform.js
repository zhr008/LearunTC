...
(function ($, learun, window) {
    ...
    $.fn.custmerform = function (formScheme) {
        var $this = $(this);
		if($this.find('.f-scroll').length == 0){
			$this.scroll();
		}
	...
	/*加载系统表单*/
	learun.sysForm = function(op, $this,callback){
		if(op.appurl){
			if($this.find('.f-scroll').length == 0){
				$this.scroll();
			}
			var $container = $this.find('.f-scroll');
			var $list = $('<div class="lr-form-container" ></div>');
			$list.append('<div class="lr-form-row lr-form-row-title"><label>'+op.name+'</label></div>');
			var $sysContainer = $('<div class="sysForm" data-url="'+op.appurl+'" ></div>');
			$list.append($sysContainer);
			learun.nav.load(op.appurl);
			learun.nav.loadhtml(learun.nav.data[op.appurl],$sysContainer, {},callback)			
			$container.append($list);
		}
	}
	
	learun.sysFormSave = function(processId,sysList,callback){
		// 保存数据
		sysFormHave = {};
		function monitorSave() {
			var _num = 0;
			var _flag = true;
			for (var i = 0, l = sysList.length; i < l; i++) {
				if (sysFormHave[i] == 0) {
					_num++;
					_flag = false;
				}
				else if (sysFormHave[i] == 1) {
					_num++;
				}
			}
			if (_num == sysList.length) {
				if (_flag) {
					callback(true);
				}
				else {
					callback(false);
					learun.layer.toast('表单数据保存失败');
				}
			}
			else {
				setTimeout(function () {
		...
		for(var i=0;i<sysList.length;i++){
			...
			sysFormHave[i] = null;
			sysPage.jsObj.save(processId,function(res,j){
				if(res){
					sysFormHave[j] = 1;
				}
				else{
