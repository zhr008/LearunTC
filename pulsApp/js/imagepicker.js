...
(function ($, learun, fui, window) {
    ...
    var _imagepicker = {
        ...
        upload: function (url, op, callback) {// 上传图片文件
            if (op.uploadUrl) {
				...
				var task = plus.uploader.createUpload(encodeURI(op.uploadUrl), 
					{ method:"POST",blocksize:0,priority:100 },
					function ( t, status ) {
						var _res = JSON.parse(t.responseText);
						console.log(_res,t.responseText);
						// 上传完成
						if ( status == 200 && _res.code === 200 ) { 
				...
				$.each(op.params,function(_key,_value){
					task.addData(_key, _value );
				});
				task.start();
    ...
    $.fn.imagepicker = function (op) {
        ...
        $this.find('.lr-imagepicker-handle').on('tap', function () {
            ...
            var op_tap = $imagePicker[0].op;
			if(op_tap._maxCount <= 0){
				learun.layer.toast('最多能选择'+op_tap.maxCount+'张图片');
				return;
			}
			

            if (op_tap.isOnlyCamera) {
                learun.camera(function (res) {
                    learun.layer.loading(true, '正在上传...');
                    // 上传文件
                    _imagepicker.upload(res, op_tap, function (isOk, _url, _op, _fileId) {
				...
				var fn_imagePick = function(){
					learun.imagePick(function (res) {
					    learun.layer.loading(true, '正在上传...');
					    _imagepicker.uploads(res, 0, op_tap, function (isOk, _url, _op, _fileId) {
					        if (isOk) {
					            _imagepicker.addImage(_url, _op, _fileId);
					        }
					    }, function () {
        ...
        $this.delegate('.lr-imagepicker-remove>i', 'tap', { op: dfop }, function (e) {
            ...
            op_remove.deleteImg && op_remove.deleteImg(fileId);
            return false;
        });

        $this.delegate('img', 'tap', { op: dfop }, function (e) {
            var op_show = e.data.op;
                    ...
                    learun.actionsheet({
                        ...
                        event: function () {
                            if (_src.indexOf('http://') !== -1) {
                                _imagepicker.down(_src, op_show);
                            }
                            else {
