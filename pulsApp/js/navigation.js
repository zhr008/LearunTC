...
(function ($, learun, fui, window) {
    ...
    // 加载js代码
    function loadjs(_page, _$pagebody, _op,callback) {
        if (_page.jsLoaded) {
            if (_page.jsObj) {
                ...
                setTimeout(function () {
					...
					callback && callback(_page.jsObj);
                }, 100);
            }
        }
        else {
            setTimeout(function () {
                loadjs(_page, _$pagebody, _op,callback);
            }, 200);
        }
    }
    // 加载html代码
    function loadhtml(_page, _$pagebody, _op,callback) {
        if (_page.htmlLoaded) {
            ...
            setTimeout(function () {
                loadhtml(_page, _$pagebody, _op,callback);
            }, 200);
        }
    }
	
	learun.nav.load = load;
	learun.nav.loadhtml = loadhtml;
