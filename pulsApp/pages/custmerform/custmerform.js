	...
	var page = {
		...
		initView: function($page) { // 初始化视图列表页
			...
			$page.find('#custmerformfn_addbtn').on('tap', function() {
				learun.nav.go({
					path: 'custmerform/form',
					title: '新增',
					type: 'right',
					param: {
						formScheme: page.formScheme,
						formSchemeId: page.formSchemeId
					}
				});
				return false;
			});
		...
		rowRender: function(_index, _item, _$item, $page) { // 渲染列表行数据
			...
			if(content0){
				title1 = content0.title;
			}
			if(content1){
				title2 = content1.title;
			}
			if(content2){
				title3 = content2.title;
			}
			...
			var _html =
				'<div class="lr-list-item">\
                                ...
                                <div class="content">\
                                    <div class="one"><div><span class="lr-tag"></span>' +
				title1 +
				'</div><div class="text"></div></div>\
                                    <div class="two"><div><span class="lr-tag"></span>' +
				title2 +
				'</div><div class="text"></div></div>\
                                    <div class="three"><div><span class="lr-tag"></span>' +
				title3 +
				'</div><div class="text"></div></div>\
                                </div>\
                            </div>';
