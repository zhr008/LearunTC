                    ...
                    if (_item.isHide != '1') {
                        var point = {
                            ...
                            event: function () {
                                ...
                                learun.nav.go({ path: 'nworkflow/audit/verify', title: headText + '【' + _item.name + '】', type: 'right', param: { processId: processId, taskId: taskId, verify: _item.code, name: _item.name, node: currentNode,isSign:_item.isSign } });
                            }
                        }
            ...
            if (data) {
                ...
                $.each(currentNode.wfForms, function (_index, _item) {
                    if (_item.formId) {
						...
						// 加载系统表单
						learun.sysForm(_item,$('#nauditcontainer'),function(jsObj){
							jsObj.setAuthorize(_item.authorize);
							jsObj.setFormData(processId);
						});
					}
                });
