(function () {
    ...
    var page = {
        ...
        create: function () {// info加签信息
			...
			learun.sysFormSave(info.childProcessId,sysList,function(formSaveRes){
				if(formSaveRes){
                    ...
                    if (cdata) {
                        ...
                        $.each(wfForms, function (_index, _item) {
                            if (_item.formId) {
								...
								// 加载系统表单
								learun.sysForm(_item,$('#chlidauditcontainer'),function(jsObj){
									jsObj.setAuthorize(_item.authorize);
									jsObj.setFormData(info.childProcessId);
								});
							}
                        });
