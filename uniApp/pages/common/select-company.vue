<template>
  <view id="contact" class="page">
    <l-banner v-model="searchText" noSearchButton placeholder="搜索(分)公司名" type="search" fill fixed />
...
export default {
  ...
  methods: {
    async init() {
      const { company: companyTable } = this.$store.state
      const { find } = this
